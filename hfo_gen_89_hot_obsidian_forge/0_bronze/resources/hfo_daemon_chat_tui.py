#!/usr/bin/env python3
"""
hfo_daemon_chat_tui.py — HFO Gen89 Daemon Chat TUI (Option B)
===============================================================
...
Schema: hfo.gen89.tui.daemon_chat.v1
...
def format_event_line(evt: dict) -> str:
    ...
    short = etype.replace("hfo.gen89.", "").replace("hfo.gen88.", "")
...
class DaemonChatApp(App):
    """HFO Gen89 Daemon Chat TUI — Option B from DSE AoA."""

    TITLE = "HFO Gen89 Daemon Chat"
    ...
    def _send_to_daemon(self, message: str, port: dict) -> None:
        ...
        context_lines = []
        for evt in recent[:5]:
            short = evt["event_type"].replace("hfo.gen89.", "")
...
def main():
    parser = argparse.ArgumentParser(
        description="HFO Gen89 Daemon Chat TUI",
    )
    parser.add_argument(
        "--port", "-p", default="P4",
