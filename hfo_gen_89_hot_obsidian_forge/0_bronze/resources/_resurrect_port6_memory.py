...
def resurrect_port6_memory():
    ...
    forge_dir = root_dir / "hfo_gen_89_hot_obsidian_forge"
    
    bronze_res = forge_dir / "0_bronze" / "resources"
    silver_res = forge_dir / "1_silver" / "resources"
    gold_res = forge_dir / "2_gold" / "resources"
    
    # Ensure silver resources directory exists
    silver_res.mkdir(parents=True, exist_ok=True)
    
    # 1. Find and move SSOT DB
    ssot_db_name = "hfo_gen89_ssot.sqlite"
    ssot_db_src = None
    ...
    pointer_file = root_dir / "hfo_gen89_pointers_blessed.json"
    if pointer_file.exists():
        ...
        if "pointers" in pointers and "ssot.db" in pointers["pointers"]:
            ...
            new_path = "hfo_gen_89_hot_obsidian_forge/1_silver/resources/hfo_gen89_ssot.sqlite"
