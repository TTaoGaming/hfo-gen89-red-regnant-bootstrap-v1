...
EVT_NPU_DISCOVERY = "hfo.gen89.kraken.npu.discovery"
EVT_GPU_ENRICHMENT = "hfo.gen89.kraken.gpu.enrichment"
EVT_LOOP_PULSE = "hfo.gen89.kraken.loop.pulse"
EVT_LOOP_HEALTH = "hfo.gen89.kraken.loop.health"
                   ...
                   AND e.created_at < (
                       SELECT COALESCE(
                           (SELECT MAX(timestamp) FROM stigmergy_events
                            WHERE event_type LIKE 'hfo.gen89.kraken.%'
    ...
    async def _emit_loop_health(self):
        ...
        try:
            ...
            loop_events = conn_ro.execute(
                "SELECT COUNT(*) FROM stigmergy_events WHERE event_type LIKE 'hfo.gen89.kraken.%'"
            ).fetchone()[0]
