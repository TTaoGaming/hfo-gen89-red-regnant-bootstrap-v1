#!/usr/bin/env python3
"""
hfo_daemon_chat_web.py - HFO Gen89 Daemon Chat Web App
========================================================
...
Schema: hfo.gen89.web.daemon_chat.v1
...
## Context
- Gen89 SSOT: ~9,861 documents, ~9M words, 10,600+ stigmergy events
...
<title>HFO Gen89 Daemon Chat</title>
<style>
:root {
  --bg: #0d1117;
  --bg2: #161b22;
...
async function pollStigmergy() {
  try {
    ...
    panel.innerHTML = events.map(evt => {
      ...
      const shortType = (evt.event_type || '').replace('hfo.gen89.', '').replace('hfo.gen88.', '');
...
def main():
    ...
    parser = argparse.ArgumentParser(description="HFO Gen89 Daemon Chat Web App")
    ...
    # Log session start
    _write_stigmergy_sync(
        "hfo.gen89.web.session.start",
        {"host": args.host, "port": args.port, "model": args.model},
        "web-session-start",
    )

    url = f"http://{args.host}:{args.port}"
    print(f"\n  HFO Gen89 Daemon Chat Web")
    print(f"  =========================")
    print(f"  URL:   {url}")
