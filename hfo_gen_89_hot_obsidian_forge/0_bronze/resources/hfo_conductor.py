...
- Emit hfo.gen89.orchestrator.scale event to stigmergy on every change
...
HFO_ROOT   = _find_root()
_RES_DIR   = HFO_ROOT / "hfo_gen_89_hot_obsidian_forge" / "0_bronze" / "resources"
...
def _run_port_tick(port_id: str, cfg: OrchestratorConfig, stats: dict) -> dict:
    ...
    try:
        ...
        _stigmergy("hfo.gen89.orchestrator.advisory", {
            "port": port_id,
        ...
        _stigmergy("hfo.gen89.orchestrator.error",
...
class Orchestrator:
    ...
    def run(self) -> None:
        ...
        _stigmergy("hfo.gen89.orchestrator.start", {
        ...
        try:
            ...
            self._write_status()
            _stigmergy("hfo.gen89.orchestrator.stop",
    ...
    def _warm_gpu_if_cold(self) -> None:
        ...
        try:
            ...
            _stigmergy("hfo.gen89.orchestrator.gpu_warm", {
                "model":          model,
    ...
    def _maybe_scale(self) -> None:
        ...
        if new != current:
            ...
            _stigmergy("hfo.gen89.orchestrator.scale", event,
                       db_path=self.cfg.db_path)
