#!/usr/bin/env python3
"""
hfo_progressive_summarizer.py — HFO Gen89 Progressive SSOT Summarizer
======================================================================
...
def write_stigmergy_event(event_type: str, subject: str, data: dict):
    ...
    payload = json.dumps({
        "specversion": "1.0",
        "type": event_type,
        "source": "hfo_progressive_summarizer_gen89_v1",
        "subject": subject,
        "time": now,
        "data": data,
    }, default=str)
    ...
    try:
        conn.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?)",
            (event_type, now, "hfo_progressive_summarizer_gen89_v1", subject, payload, content_hash)
        )
        conn.commit()
...
def run_daemon(model_tier: str = "flash_25", batch_size: int = 20,
    ...
    # Write summary event
    write_stigmergy_event(
        "hfo.gen89.p6.enrichment.daemon_complete",
