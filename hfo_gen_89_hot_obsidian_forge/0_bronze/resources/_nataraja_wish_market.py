...
def create_nataraja_wish():
    ...
    event_data = {
        "specversion": "1.0",
        "id": event_id,
        "type": "hfo.gen89.p7.wish.nataraja_market",
    ...
    cur.execute(
        """INSERT INTO stigmergy_events 
           ...
           VALUES (?, ?, ?, ?, ?, ?)""",
        (
            "hfo.gen89.p7.wish.nataraja_market",
            now,
    ...
    for p in PANTHEON:
        ...
        cur.execute(
            """INSERT INTO stigmergy_events
               ...
               VALUES (?, ?, ?, ?, ?, ?)""",
            (
                f"hfo.gen89.p7.wish.nataraja_summon.{p['port'].lower()}",
                now,
                "ttao_operator_via_p4_red_regnant",
    ...
    cur.execute(
        """SELECT id, event_type, subject FROM stigmergy_events 
           WHERE event_type LIKE 'hfo.gen89.p7.wish.nataraja_summon.%'
           AND timestamp = ?""",
        (now,),
    )
