...
import { PointerUpdateSchema, PointerCoastSchema } from './schemas';
...
export class W3CPointerFabric implements Plugin {
    ...
    private boundOnPointerUpdate!: (data: any) => void;
    private boundOnPointerCoast!: (data: any) => void;
    ...
    constructor(config: Partial<PointerFabricConfig> = {}) {
        this.config = {
            ...
            lookaheadSteps: 3, // Default to 3 steps of prediction for Level 3
            smoothingR: 1,
            smoothingQ: 1,
            ...config
        };
    ...
    private onPointerUpdate(data: any) {
        // Runtime Syntactic Enforcement (Zod)
        const parsed = PointerUpdateSchema.parse(data);
        this.processLandmark(parsed.handId, parsed.x, parsed.y, parsed.isPinching);
    }

    private onPointerCoast(data: any) {
        // Runtime Syntactic Enforcement (Zod)
        const parsed = PointerCoastSchema.parse(data);
        if (parsed.destroy) {
            ...
            this.removeHand(parsed.handId);
        } else {
            this.coastLandmark(parsed.handId, parsed.isPinching);
        }
    }
    ...
    private elementFromPoint(x: number, y: number): Element | null {
        ...
        const iframeEl = stack.find(el => el.tagName.toLowerCase() === 'iframe');
        if (iframeEl) return iframeEl;

        // Pass 2: first element that can receive pointer events
        const interactive = stack.find(el => window.getComputedStyle(el).pointerEvents !== 'none');
    ...
    private firePointerEvent(
        ...
        const eventInit: PointerEventInit = {
            pointerId: pointerId,
            pointerType: 'touch', // 'touch' is usually best for gesture interfaces
            isPrimary: true,
            clientX: clientX,
            clientY: clientY,
        ...
        const createSubEvent = (raw: { x: number, y: number, time: number }) => {
            const subEvent = new PointerEvent(type, {
                ...eventInit,
                clientX: raw.x,
                clientY: raw.y,
                screenX: raw.x,
        ...
        (event as any).getCoalescedEvents = () => coalescedEvents;
        (event as any).getPredictedEvents = () => predictedEvents;
    ...
    public removeHand(handId: number) {
        ...
        if (state) {
            ...
            if (state.isDown) {
                const target = this.elementFromPoint(state.x, state.y) || document.body;
