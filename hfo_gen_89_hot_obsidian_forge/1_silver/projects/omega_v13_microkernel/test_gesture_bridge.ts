...
bridge.processFrame([
    { handId: 0, x: 0.2, y: 0.5, gesture: 'open_palm', confidence: 0.9 },
    { handId: 1, x: 0.8, y: 0.5, gesture: 'open_palm', confidence: 0.9 }
]);
...
bridge.processFrame([
    { handId: 0, x: 0.2, y: 0.5, gesture: 'pointer_up', confidence: 0.9 },
    { handId: 1, x: 0.8, y: 0.5, gesture: 'open_palm', confidence: 0.9 }
]);
...
bridge.processFrame([
    { handId: 1, x: 0.81, y: 0.51, gesture: 'open_palm', confidence: 0.9 }
]);
console.log(`Hand 1 State: ${fsmPlugin.getHandState(1)}`); // Should be READY
...
const mutexBridge = new GestureBridge(mutexAdapter);
...
const mutexFrames: RawHandData[][] = [
    ...
    [
        ...
        { handId: 2, x: 0.5, y: 0.5, gesture: 'open_palm', confidence: 0.9 }
    ],
    // Frame 1: Hand 2 commits (acquires lock)
    [
        { handId: 1, x: 0.1, y: 0.1, gesture: 'open_palm', confidence: 0.9 },
        { handId: 2, x: 0.5, y: 0.5, gesture: 'pointer_up', confidence: 0.9 }
    ],
    // Frame 2: Hand 1 tries to commit (ignored because Hand 2 has lock)
    [
        { handId: 1, x: 0.1, y: 0.1, gesture: 'pointer_up', confidence: 0.9 },
        { handId: 2, x: 0.5, y: 0.5, gesture: 'pointer_up', confidence: 0.9 }
    ]
];
