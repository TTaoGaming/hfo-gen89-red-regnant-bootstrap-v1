...
(() => {
  ...
  var PathAbstractionLayer = class {
    constructor() {
  ...
  // gesture_fsm.ts
  var GestureFSM = class {
    constructor() {
      this.state = "IDLE";
      // Schmitt Trigger Thresholds
      this.conf_high = 0.64;
      this.conf_low = 0.5;
      // Leaky Bucket Dwell Limits
      this.dwell_limit_ready = 15;
      this.dwell_limit_commit = 10;
      // Current State Variables
      this.current_confidence = 0;
      this.dwell_accumulator = 0;
      // Coast Timeout
      this.coast_ticks = 0;
      this.coast_timeout_limit = 30;
    }
    // frames before total loss
    /**
     ...
     */
    processFrame(gesture, confidence, x2 = -1, y2 = -1) {
      this.current_confidence = confidence;
      if (this.state.includes("COAST")) {
        this.coast_ticks++;
        if (this.coast_ticks >= this.coast_timeout_limit) {
          this.state = "IDLE";
          this.dwell_accumulator = 0;
          return;
        }
      ...
      switch (this.state) {
        case "IDLE":
          this.handleIdle(gesture);
          break;
        case "IDLE_COAST":
          this.handleIdleCoast();
          break;
        case "READY":
          this.handleReady(gesture);
          break;
        case "READY_COAST":
          this.handleReadyCoast();
          break;
        case "COMMIT_POINTER":
          this.handleCommitPointer(gesture);
          break;
        case "COMMIT_COAST":
    ...
    handleReady(gesture) {
      ...
      if (this.dwell_accumulator >= this.dwell_limit_commit) {
        this.state = "COMMIT_POINTER";
        this.dwell_accumulator = 0;
      }
    }
    handleReadyCoast() {
    ...
    handleCommitPointer(gesture) {
      if (this.current_confidence < this.conf_low) {
        this.state = "COMMIT_COAST";
        return;
      }
     ...
     */
    forceCoast() {
      if (this.state === "IDLE") this.state = "IDLE_COAST";
      else if (this.state === "READY") this.state = "READY_COAST";
      else if (this.state === "COMMIT_POINTER") this.state = "COMMIT_COAST";
    }
  };

  // gesture_fsm_plugin.ts
  var GestureFSMPlugin = class {
    constructor() {
      this.name = "GestureFSMPlugin";
      this.version = "1.0.0";
      this.fsmInstances = /* @__PURE__ */ new Map();
    }
    init(context) {
    ...
    onStillnessDetected(data) {
      const fsm = this.fsmInstances.get(data.handId);
      if (fsm) {
        fsm.forceCoast();
      }
    }
    onFrameProcessed(hands) {
      const currentHandIds = /* @__PURE__ */ new Set();
      for (const hand of hands) {
        currentHandIds.add(hand.handId);
        if (!this.fsmInstances.has(hand.handId)) {
          this.fsmInstances.set(hand.handId, new GestureFSM());
        }
        const fsm = this.fsmInstances.get(hand.handId);
        const previousState = fsm.state;
        fsm.processFrame(hand.gesture, hand.confidence, hand.x, hand.y);
  ...
  var AudioEnginePlugin = class {
    constructor() {
      this.name = "AudioEnginePlugin";
      this.version = "1.0.0";
      this.audioContext = null;
      this.clickDownBuffer = null;
      this.clickUpBuffer = null;
    ...
    async onAudioUnlock() {
      if (!this.audioContext) {
        try {
          ...
          console.warn("[AudioEngine] AudioContext not supported or failed to initialize", e2);
        }
      } else if (this.audioContext.state === "suspended") {
        this.audioContext.resume().then(() => {
          console.log("[AudioEngine] AudioContext unlocked and resumed");
        }).catch((e2) => {
          console.warn("[AudioEngine] Failed to resume AudioContext", e2);
        });
      }
    }
    async loadSounds() {
    ...
    synthesizeClick(isDown) {
      ...
      for (let i2 = 0; i2 < buffer.length; i2++) {
        const t2 = i2 / sampleRate;
        if (isDown) {
          const clickEnv = Math.exp(-t2 * 800);
          const clickOsc = Math.sin(2 * Math.PI * 3500 * t2) * clickEnv;
          const clackDelay = 0.01;
          let clackOsc = 0;
          if (t2 > clackDelay) {
            const clackEnv = Math.exp(-(t2 - clackDelay) * 200);
            clackOsc = Math.sin(2 * Math.PI * 400 * (t2 - clackDelay)) * clackEnv;
          }
          const noise = (Math.random() * 2 - 1) * Math.exp(-t2 * 300) * 0.2;
          data[i2] = (clickOsc * 0.4 + clackOsc * 0.6 + noise) * 0.5;
        } else {
          const clickEnv = Math.exp(-t2 * 600);
          const clickOsc = Math.sin(2 * Math.PI * 2500 * t2) * clickEnv;
          const topOutDelay = 0.015;
          let topOutOsc = 0;
          if (t2 > topOutDelay) {
            const topOutEnv = Math.exp(-(t2 - topOutDelay) * 150);
            topOutOsc = Math.sin(2 * Math.PI * 500 * (t2 - topOutDelay)) * topOutEnv;
          }
          const noise = (Math.random() * 2 - 1) * Math.exp(-t2 * 200) * 0.1;
          data[i2] = (clickOsc * 0.3 + topOutOsc * 0.5 + noise) * 0.4;
        }
      }
      return buffer;
    }
  ...
  // visualization_plugin.ts
  var VisualizationPlugin = class {
    constructor() {
      this.name = "VisualizationPlugin";
      this.version = "1.0.0";
      this.container = null;
      this.handElements = /* @__PURE__ */ new Map();
    }
    init(context) {
      this.context = context;
    ...
    onPointerUpdate(data) {
      ...
      if (data.rawLandmarks && this.container) {
        ...
        const scale = window.omegaOverscanScale || 1;
        ...
        const confScore = data.confidence !== void 0 ? data.confidence.toFixed(2) : "N/A";
        textOverlay.innerText = `${gestureName} (${confScore})`;
      }
    }
    onStateChange(data) {
      const el = this.getOrCreateHandElement(data.handId);
      const innerDot = el.querySelector(".inner-dot");
  ...
  var SymbioteInjectorPlugin = class {
    constructor() {
      this.name = "SymbioteInjectorPlugin";
      this.version = "1.0.0";
      // Track previous pinch state to emit pointerdown/pointerup
      this.isPinchingMap = /* @__PURE__ */ new Map();
    }
    init(context) {
      this.context = context;
      this.context.eventBus.subscribe("POINTER_UPDATE", this.onPointerUpdate.bind(this));
    ...
    onPointerUpdate(data) {
      const { handId, x: x2, y: y2, isPinching } = data;
      const screenX = x2 * window.innerWidth;
      const screenY = y2 * window.innerHeight;
  ...
  var j = "function" == typeof Symbol && "symbol" == typeof /* @__PURE__ */ Symbol();
  ...
  var H = V(void 0, "0di");
  var W = V(void 0, "1oa");
  var z = V(void 0, /* @__PURE__ */ Symbol());
  var K = V(void 0, "0ub");
  var Y = V(void 0, "0ubs");
  ...
  async function bootstrap() {
    ...
    startBtn.innerText = "Tap to Calibrate Camera";
    ...
    startBtn.style.boxShadow = "0 4px 8px rgba(0,0,0,0.2)";
    ...
    const PROCESS_INTERVAL_MS = 1e3 / 15;
    startBtn.onclick = async () => {
      console.log("Camera calibrated, Audio context unlocked");
      globalEventBus.publish("AUDIO_UNLOCK", null);
      startBtn.remove();
      try {
        const stream = await navigator.mediaDevices.getUserMedia({ video: true });
        videoElement.srcObject = stream;
        const vision = await na.forVisionTasks(
          "https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.3/wasm"
        );
        handLandmarker = await Uc.createFromOptions(vision, {
          baseOptions: {
        ...
        console.error("Error accessing camera or loading MediaPipe:", err);
      }
    };
    document.body.appendChild(startBtn);
    function calculateAngle(a2, b2, c2) {
      const ba2 = { x: a2.x - b2.x, y: a2.y - b2.y, z: a2.z - b2.z };
      const bc2 = { x: c2.x - b2.x, y: c2.y - b2.y, z: c2.z - b2.z };
      const dotProduct = ba2.x * bc2.x + ba2.y * bc2.y + ba2.z * bc2.z;
      const magBA = Math.sqrt(ba2.x * ba2.x + ba2.y * ba2.y + ba2.z * ba2.z);
      const magBC = Math.sqrt(bc2.x * bc2.x + bc2.y * bc2.y + bc2.z * bc2.z);
      if (magBA * magBC === 0) return 0;
      let angle = Math.acos(dotProduct / (magBA * magBC));
      return angle * (180 / Math.PI);
    }
    function getFingerCurlScore(mcp, pip, dip) {
      const angle = calculateAngle(mcp, pip, dip);
      let score = (180 - angle) / 90;
      return Math.max(0, Math.min(1, score));
    }
    const gestureBuckets = {};
    ...
    function predictWebcam() {
      ...
      if (lastVideoTime !== videoElement.currentTime && startTimeMs - lastProcessTime > PROCESS_INTERVAL_MS) {
        lastVideoTime = videoElement.currentTime;
        lastProcessTime = startTimeMs;
        const results = handLandmarker.detectForVideo(videoElement, startTimeMs);
        if (results.landmarks && results.landmarks.length > 0) {
          const handsData = results.landmarks.map((landmarks, index) => {
            const indexCurl = getFingerCurlScore(landmarks[5], landmarks[6], landmarks[7]);
            const middleCurl = getFingerCurlScore(landmarks[9], landmarks[10], landmarks[11]);
            const ringCurl = getFingerCurlScore(landmarks[13], landmarks[14], landmarks[15]);
            const pinkyCurl = getFingerCurlScore(landmarks[17], landmarks[18], landmarks[19]);
            const indexKnuckle = landmarks[5];
            const pinkyKnuckle = landmarks[17];
            const palmWidth = Math.sqrt(
              Math.pow(indexKnuckle.x - pinkyKnuckle.x, 2) + Math.pow(indexKnuckle.y - pinkyKnuckle.y, 2) + Math.pow(indexKnuckle.z - pinkyKnuckle.z, 2)
            );
            const thumbTip = landmarks[4];
            const palmCenter = landmarks[9];
            const thumbDist = Math.sqrt(
              Math.pow(thumbTip.x - palmCenter.x, 2) + Math.pow(thumbTip.y - palmCenter.y, 2) + Math.pow(thumbTip.z - palmCenter.z, 2)
            );
            const normalizedThumbDist = thumbDist / palmWidth;
            const thumbScore = Math.max(0, Math.min(1, (2 - normalizedThumbDist) / 1));
            const middleTip = landmarks[12];
            const thumbMiddleDist = Math.sqrt(
              Math.pow(thumbTip.x - middleTip.x, 2) + Math.pow(thumbTip.y - middleTip.y, 2) + Math.pow(thumbTip.z - middleTip.z, 2)
            );
            const normalizedThumbMiddleDist = thumbMiddleDist / palmWidth;
            const thumbMiddleScore = Math.max(0, Math.min(1, (1.5 - normalizedThumbMiddleDist) / 1));
            ...
            if (pointerUpScore > maxScore && pointerUpScore > 0.6) {
              gesture = "pointer_up";
              maxScore = pointerUpScore;
            }
            if (fistScore > maxScore && fistScore > 0.6) {
              gesture = "closed_fist";
              maxScore = fistScore;
            }
            ...
            const scale = window.omegaOverscanScale || 1;
            const offset = (1 - 1 / scale) / 2;
            const mappedX = (indexTip.x - offset) * scale;
            const mappedY = (indexTip.y - offset) * scale;
            return {
              handId: index,
              gesture,
              confidence: maxScore,
              x: mappedX,
              // Use mapped index tip for pointer location
              y: mappedY,
              rawLandmarks: landmarks
              // Pass raw landmarks for visualization
            };
          });
