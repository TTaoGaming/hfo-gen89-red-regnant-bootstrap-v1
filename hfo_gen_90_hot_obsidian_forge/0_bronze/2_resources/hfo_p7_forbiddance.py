#!/usr/bin/env python3
"""
hfo_p7_forbiddance.py — P7 Spider Sovereign FORBIDDANCE Spell (Gen90)
======================================================================
...
═══════════════════════════════════════════════
  ...
  TIER 3 — GOVERNANCE:
    ...
    Scenario: SSOT medallion consistency
      ...
      Then  verify all docs tagged bronze (expected for Gen90)
...
Event Types:
    hfo.gen90.p7.forbiddance.ward          — Ward check performed
    hfo.gen90.p7.forbiddance.violation     — Boundary violation detected
    hfo.gen90.p7.forbiddance.clean         — All boundaries respected
    hfo.gen90.p7.forbiddance.hooks         — Git hook status checked
    hfo.gen90.p7.forbiddance.medallion     — SSOT medallion audit
...
ALLOWED_ROOT_ENTRIES = {
    ...
    "hfo_gen90_pointers_blessed.json",
...
def check_layer_boundaries() -> tuple[list[str], list[str]]:
    ...
    silver_resources = FORGE_ROOT / "1_silver" / "2_resources"
    ...
    gold_resources = FORGE_ROOT / "2_gold" / "2_resources"
    ...
    hfo_resources = FORGE_ROOT / "3_hyper_fractal_obsidian" / "2_resources"
...
def check_ssot_medallions() -> tuple[list[str], list[str]]:
    ...
    SBE:
      Given  all Gen90 documents should be bronze medallion
        ...
        for row in rows:
            if row["cnt"] > 0:
                violations.append(
                    ...
                    f"(all Gen90 docs should be bronze)"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="P7 Spider Sovereign — FORBIDDANCE Spell (Gen90)",
