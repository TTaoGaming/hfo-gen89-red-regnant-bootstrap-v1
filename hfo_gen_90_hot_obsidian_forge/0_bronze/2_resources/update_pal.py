...
p_in = pathlib.Path("hfo_gen_90_hot_obsidian_forge/0_bronze/demoted_hfo/archives/hfo_gen90_pointers_blessed.json")
...
for key, entry in data["pointers"].items():
    if isinstance(entry, dict):
        path = entry["path"]
        path = path.replace("hfo_gen_90_hot_obsidian_forge", "hfo_gen_90_hot_obsidian_forge")
        path = path.replace("hfo_gen90_ssot.sqlite", "hfo_gen90_ssot.sqlite")
        path = path.replace("0_bronze/resources", "0_bronze/2_resources")
        path = path.replace("1_silver/resources", "1_silver/2_resources")
        path = path.replace("2_gold/2_resources", "2_gold/2_resources")
