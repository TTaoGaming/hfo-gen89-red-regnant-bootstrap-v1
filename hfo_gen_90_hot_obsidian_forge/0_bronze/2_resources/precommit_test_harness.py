...
KEY_PATTERNS = re.compile(r"-----BEGIN (RSA |EC |OPENSSH |DSA |ENCRYPTED )?PRIVATE\x20KEY-----|-----BEGIN\x20CERTIFICATE-----|PRIVATE\x20KEY")

ALLOWED_ROOT = re.compile(r"^(AGENTS.md|\.env\.example|\.gitignore|\.gitattributes|\.githooks|\.github|\.vscode|hfo_gen90_pointers_blessed\.json|hfo_gen_90_hot_obsidian_forge|LICENSE|README\.md|\.git)$")
    ...
    try:
        with open(path, "r", encoding="utf-8", errors="ignore") as f:
            ...
            if KEY_PATTERNS.search(content):
                return False, f"PRIVATE_KEY / CERT DETECTED IN: {path}"
    except Exception:
        pass
    return True, None

def check_medallion_boundary(path):
    if path.startswith("hfo_gen_90_hot_obsidian_forge/1_silver/") or path.startswith("hfo_gen_90_hot_obsidian_forge/2_gold/") or path.startswith("hfo_gen_90_hot_obsidian_forge/3_hyper_fractal_obsidian/"):
