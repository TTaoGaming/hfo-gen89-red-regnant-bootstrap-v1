#!/usr/bin/env python3
"""
HFO Gen90 Fleet Uptime Monitor — Hourly % Uptime Per Port
==========================================================
...
Event-to-Port Mapping:
  P0 OBSERVE  — hfo.gen90.daemon.{start,stop,advisory,swarm_*,model_*}
  P1 BRIDGE   — hfo.gen90.daemon.research
  P2 SHAPE    — hfo.gen90.daemon.{deep_analysis,code_eval}, hfo.gen90.chimera.*
  P3 INJECT   — hfo.gen90.daemon.{enrich.*,patrol,port_assign}
  P4 DISRUPT  — hfo.gen90.singer.*
  P5 IMMUNIZE — hfo.gen90.dancer.*
  P6 ASSIMILATE — hfo.gen90.{devourer.*,kraken.*}
  P7 NAVIGATE — hfo.gen90.summoner.*
"""
...
SSOT_DB = FORGE_ROOT / "2_gold" / "2_resources" / "hfo_gen90_ssot.sqlite"
...
PORT_CONFIG = {
    "P0": {
        ...
        "event_patterns": [
            "hfo.gen90.daemon.start",
            "hfo.gen90.daemon.stop",
            "hfo.gen90.daemon.error",
            "hfo.gen90.daemon.advisory",
            "hfo.gen90.daemon.swarm%",
            "hfo.gen90.daemon.model%",
        ],
    ...
    "P2": {
        "name": "SHAPE (Shaper)",
        "event_patterns": [
            "hfo.gen90.daemon.deep_analysis",
            "hfo.gen90.daemon.code_eval",
            "hfo.gen90.chimera.%",
        ],
        "expected_per_hour": 4,
    ...
    "P3": {
        ...
        "event_patterns": [
            "hfo.gen90.daemon.enrich%",
            "hfo.gen90.daemon.patrol",
            "hfo.gen90.daemon.enrich.port_assign",
        ],
        "expected_per_hour": 30,
...
def run_monitor(interval_min: float = 10, duration_min: float = 60, once: bool = False):
    ...
    print("=" * 92)
    print("  HFO Gen90 Fleet Uptime Monitor")
...
def main():
    parser = argparse.ArgumentParser(
        description="HFO Gen90 Fleet Uptime Monitor — per-port hourly %% uptime",
