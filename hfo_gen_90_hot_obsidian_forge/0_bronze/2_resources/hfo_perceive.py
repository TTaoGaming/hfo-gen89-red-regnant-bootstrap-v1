...
Writes a hfo.gen90.prey8.perceive CloudEvent to stigmergy_events.
...
def perceive(probe: str = None, port: int = None, json_output: bool = False) -> dict:
    ...
    event = make_cloudevent(
        event_type="hfo.gen90.prey8.perceive",
        subject="prey-perceive",
        data=event_data,
        source="hfo_perceive.py_gen90_v1",
    )
...
def main():
    ...
    parser.add_argument("--port", type=int, default=None, choices=range(8),
                        help="Octree port (0-7) to load context capsule for")
    parser.add_argument("--json", "-j", action="store_true",
                        help="Output machine-readable JSON")
    args = parser.parse_args()
    perceive(probe=args.probe, port=args.port, json_output=args.json)
