...
=========================================================================
v1.0 | Gen90 | Port: P7 NAVIGATE (orchestration) | Medallion: bronze
...
PORT ISOLATION (least-privilege):
    P2 SHAPE:     Can write hfo.gen90.chimera.* events. Model: gemma3:4b.
                  Cannot write to gold/silver. Gated by P5.
    P4 DISRUPT:   Can write hfo.gen90.p4.singer.*, hfo.gen90.p4.song_prospector.* events.
                  Singer: no LLM (pure SSOT scan). Prospector: gemma3:4b.
    P5 IMMUNIZE:  Can write hfo.gen90.p5.* events + audit all other ports.
                  Model: phi4:14b (high intelligence). Ring 0/1/2 gates.
    P6 ASSIMILATE: Already running. Writes hfo.gen90.kraken.* events.
...
HFO_ROOT = _find_root()
FORGE = HFO_ROOT / "hfo_gen_90_hot_obsidian_forge"
DB_PATH = FORGE / "2_gold" / "2_resources" / "hfo_gen90_ssot.sqlite"
BRONZE_RESOURCES = FORGE / "0_bronze" / "2_resources"
...
def check_p6_swarm_health() -> tuple[bool, str]:
    ...
    events = _read_recent_events("hfo.gen90.kraken.%", limit=5)
    if not events:
...
class SandboxSupervisor:
    ...
    def phase0_preflight(self) -> bool:
        ...
        if not passed and not self.dry_run:
            _write_stigmergy(
                "hfo.gen90.sandbox.preflight_failed",
                {"checks": checks},
        ...
        _write_stigmergy(
            "hfo.gen90.sandbox.preflight_passed",
            {
                "checks": checks,
                "port_models": {
    ...
    def phase2_boot_p4(self) -> bool:
        ...
        if singer_ok:
            _write_stigmergy(
                "hfo.gen90.sandbox.p4_singer_boot",
                {
        ...
        if self.enable_prospector:
            ...
            if prospector_ok:
                _write_stigmergy(
                    "hfo.gen90.sandbox.p4_prospector_boot",
                    {
        ...
        if p5_alive and p4_alive:
            ...
            _write_stigmergy(
                "hfo.gen90.sandbox.nataraja_activated",
                ...
                if daemon.process:
                    if daemon.process.poll() is not None:
                        ...
                        if old_state == PortState.HEALTHY:
                            ...
                            _write_stigmergy(
                                "hfo.gen90.sandbox.daemon_death",
                                {
                                    "port_key": key,
    ...
    def boot(self, ports: Optional[list[str]] = None) -> bool:
        ...
        # Boot complete
        _write_stigmergy(
            "hfo.gen90.sandbox.boot_complete",
            {
...
def show_status_from_stigmergy():
    ...
    events = _read_recent_events("hfo.gen90.sandbox.%", limit=20)
    ...
    for evt in events[:15]:
        ts = evt["timestamp"][:19]
        etype = evt["event_type"].replace("hfo.gen90.sandbox.", "")
        data = evt.get("data", {})
...
def main():
    ...
    if args.stop:
        _write_stigmergy(
            "hfo.gen90.sandbox.stop_requested",
