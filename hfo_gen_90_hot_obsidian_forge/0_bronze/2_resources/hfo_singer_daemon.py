#!/usr/bin/env python3
"""
hfo_singer_daemon.py — SINGER OF STRIFE AND SPLENDOR Daemon (Gen90)
=====================================================================
...
Stigmergy Event Types (port-sharded to P4):
  hfo.gen90.p4.singer.strife       — A SONG_OF_STRIFE observation
  hfo.gen90.p4.singer.splendor     — A SONG_OF_SPLENDOR observation
  hfo.gen90.p4.singer.requested    — A song requested by an agent via PREY8 yield (strange loop)
  hfo.gen90.p4.singer.health       — SSOT health dashboard snapshot
  hfo.gen90.p4.singer.heartbeat    — Daemon alive signal
  hfo.gen90.p4.singer.error        — Daemon self-reported error (recursive STRIFE)

  Port-sharding convention: hfo.gen90.p{N}.{daemon}.{event}
...
Token Economy (hardcoded SPLENDOR facts):
  ...
  88 generations (Gen01 → Gen90 → Gen90 consolidated)
...
def _load_pointers() -> dict:
    ...
    if not POINTERS_FILE.exists():
        raise FileNotFoundError(
            ...
            "The PAL system requires hfo_gen90_pointers_blessed.json at HFO_ROOT."
        )
...
FORGE_BRONZE = HFO_ROOT / "hfo_gen_90_hot_obsidian_forge" / "0_bronze"
...
P4_EVENT_PREFIX = "hfo.gen90.p4.singer"
...
def write_stigmergy_event(
    ...
    subject: str,
    data: dict,
    source: str = "hfo_singer_daemon_gen90",
) -> str:
...
class SingerOfStrifeAndSplendor:
    ...
    def sing_requested(self, conn_rw: sqlite3.Connection, requests: list[dict]) -> int:
        """
        Emit hfo.gen90.singer.requested events for agent song requests.
...
def main():
    parser = argparse.ArgumentParser(
        description="SINGER OF STRIFE AND SPLENDOR — P4 Red Regnant Daemon (Gen90)",
