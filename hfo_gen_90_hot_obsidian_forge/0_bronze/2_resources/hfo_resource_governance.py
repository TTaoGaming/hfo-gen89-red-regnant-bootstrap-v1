...
Event Types:
    hfo.gen90.governance.overutilized   — Sustained high resource usage
    hfo.gen90.governance.underutilized  — Sustained idle resources
    hfo.gen90.governance.rebalance      — Recommended action
    hfo.gen90.governance.budget_breach  — VRAM over budget sustained
    hfo.gen90.governance.npu_idle       — NPU available but unused
    hfo.gen90.governance.heartbeat      — Periodic governance status
...
class RegimeDetector:
    ...
    def analyze(self, buf: SampleBuffer) -> list[dict]:
        ...
        events += self._check_regime(
            ...
            event_type="hfo.gen90.governance.overutilized",
        ...
        events += self._check_regime(
            ...
            event_type="hfo.gen90.governance.budget_breach",
        ...
        events += self._check_regime(
            ...
            event_type="hfo.gen90.governance.underutilized",
        ...
        if buf.latest() and buf.latest().npu_available:
            ...
            if npu_idle_pct >= REGIME_THRESHOLD_PCT:
                events += self._check_regime(
                    ...
                    event_type="hfo.gen90.governance.npu_idle",
        ...
        try:
            ...
            row = conn.execute(
                """SELECT COUNT(*) FROM stigmergy_events
                   WHERE event_type = 'hfo.gen90.swarm.embedding'
        ...
        while self._running:
            ...
            try:
                ...
                if not self.dry_run:
                    ts = datetime.now(timezone.utc).strftime("%H%M%S")
                    _write_gov_event(
                        "hfo.gen90.governance.heartbeat",
    ...
    try:
        ...
        rows = conn.execute(
            """SELECT event_type, timestamp, subject FROM stigmergy_events
               WHERE event_type LIKE 'hfo.gen90.governance.%'
