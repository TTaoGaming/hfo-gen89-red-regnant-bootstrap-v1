#!/usr/bin/env python3
"""
hfo_background_daemon.py — HFO Gen90 Background Process Daemon
===============================================================
...
BRONZE_DIR = HFO_ROOT / "hfo_gen_90_hot_obsidian_forge" / "0_bronze"
...
def write_stigmergy_event(conn: sqlite3.Connection, event_type: str,
                          ...
                          source: str = "hfo_background_daemon_gen90"):
    ...
    async def run_once(self) -> dict:
        ...
        for row in missing_bluf:
            ...
            try:
                ...
                write_stigmergy_event(
                    wdb, "hfo.gen90.daemon.enrich.bluf",
                    f"doc-{row['id']}", {
                        "doc_id": row['id'],
                        "doc_title": row['title'],
                        "bluf": bluf.strip()[:500],
            ...
            try:
                ...
                if doc_type in valid_types:
                    ...
                    wdb.execute("UPDATE documents SET doc_type = ? WHERE id = ?",
                               (doc_type, row['id']))
                    write_stigmergy_event(
                        wdb, "hfo.gen90.daemon.enrich.classify",
                        f"doc-{row['id']}", {
                            "doc_id": row['id'],
                            "doc_title": row['title'],
                            "doc_type": doc_type,
...
class CodeGenEvalTask:
    ...
    async def run_once(self) -> dict:
        ...
        try:
            ...
            wdb = get_db_readwrite()
            write_stigmergy_event(
                wdb, "hfo.gen90.daemon.code_eval",
                f"file-{target.name}", {
                    "file": target.name,
    ...
    async def run_once(self) -> dict:
        ...
        orphans = db.execute(
            """SELECT se1.id, se1.timestamp, se1.data_json
               ...
               WHERE se1.event_type = 'hfo.gen90.prey8.perceive'
               AND se1.timestamp > datetime('now', '-48 hours')
               AND NOT EXISTS (
                   SELECT 1 FROM stigmergy_events se2
                   WHERE se2.event_type = 'hfo.gen90.prey8.yield'
        ...
        for row in unported:
            ...
            try:
                ...
                wdb.execute("UPDATE documents SET port = ? WHERE id = ?",
                           (port, row['id']))
                write_stigmergy_event(
                    wdb, "hfo.gen90.daemon.enrich.port_assign",
                    f"doc-{row['id']}", {
                        "doc_id": row['id'],
...
class DeepAnalysisTask:
    ...
    async def run_once(self) -> dict:
        ...
        doc = db.execute(
            """SELECT d.id, d.title, d.bluf, d.content, d.source, d.port,
               ...
               AND d.id NOT IN (
                   ...
                   WHERE se.event_type = 'hfo.gen90.daemon.deep_analysis'
        ...
        try:
            ...
            wdb = get_db_readwrite()
            write_stigmergy_event(
                wdb, "hfo.gen90.daemon.deep_analysis",
                f"doc-{doc['id']}", {
                    "doc_id": doc['id'],
...
class BackgroundDaemon:
    ...
    def _print_banner(self):
        print("=" * 60)
        print("  HFO Gen90 — Background Daemon")
        print("=" * 60)
...
def show_status():
    ...
    print("=" * 60)
    print("  HFO Gen90 — Background Daemon Status")
    print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description="HFO Gen90 Background Daemon — continuous SSOT enrichment"
    )
    parser.add_argument(
