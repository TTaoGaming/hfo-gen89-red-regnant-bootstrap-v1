...
=============================================================
v1.0 | Gen90 | Port: P5 IMMUNIZE | Commander: Pyre Praetorian
    ...
    async def analyze(self, prompt: str, model: str = "lite_25") -> str:
        ...
        config = types.GenerateContentConfig(
            ...
            system_instruction=(
                ...
                "of HFO Gen90. Analyze the provided system state for anomalies, "
            ...
            async with httpx.AsyncClient(timeout=120) as client:
                response = await client.post(
                    ...
                    json={
                        ...
                        "prompt": prompt,
                        "system": (
                            "You are P5 Pyre Praetorian — the immune system of HFO Gen90. "
    ...
    async def run_once(self) -> dict:
        ...
        try:
            ...
            if len(gate_blocks) >= GATE_BLOCK_STORM_THRESHOLD:
                ...
                write_p5_event(
                    "hfo.gen90.p5.anomaly.gate_block_storm",
                    "P5-ANOMALY-A1",
            ...
            if len(tampers) >= TAMPER_CLUSTER_THRESHOLD:
                ...
                result["anomalies_found"] += 1
                write_p5_event(
                    "hfo.gen90.p5.anomaly.tamper_cluster",
                    "P5-ANOMALY-A2",
            ...
            if WATCHDOG_AVAILABLE:
                ...
                if unknown_agents:
                    ...
                    result["anomalies_found"] += 1
                    write_p5_event(
                        "hfo.gen90.p5.anomaly.agent_impersonation",
                        "P5-ANOMALY-A7",
        ...
        try:
            ...
            row = conn.execute(
                """SELECT data_json FROM stigmergy_events
                   WHERE event_type = 'hfo.gen90.singer.strife'
