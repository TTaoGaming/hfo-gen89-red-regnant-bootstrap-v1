#!/usr/bin/env python3
"""
hfo_singer_ai_daemon.py — Fast AI Singer v1.0 — P4 Red Regnant (Gen90)
=========================================================================
...
Event Types:
  hfo.gen90.singer.strife     — AI-selected antipattern (balancing signal)
  hfo.gen90.singer.splendor   — AI-selected pattern (reinforcing signal)
  hfo.gen90.singer.heartbeat  — Daemon alive pulse
  hfo.gen90.singer.health     — SSOT health snapshot (every Nth cycle)
  hfo.gen90.singer.error      — Self-reported error (recursive strife)
...
def ollama_generate(
    ...
    Resource governor gate: waits for GPU headroom (80% ceiling) before calling.
    keep_alive=0s ensures phi4 / large models are evicted from VRAM after each call.
    """
    # ── Resource governor gate (hard 80% VRAM ceiling) ─────────────────────
    if _RESOURCE_GOVERNOR_AVAILABLE:
        ok = _wait_gpu_headroom(caller="singer_ollama", verbose=False)
        if not ok:
