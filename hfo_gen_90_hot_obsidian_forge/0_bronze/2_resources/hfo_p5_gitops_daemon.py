...
def _resolve_pointer(key: str) -> Path:
    pf = HFO_ROOT / "hfo_pointers.py"
    if pf.exists():
        try:
            import importlib.util
            spec = importlib.util.spec_from_file_location("hfo_pointers", pf)
            hfo_pointers = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(hfo_pointers)
            return Path(hfo_pointers.resolve(key))
        except Exception:
            pass
    # Fallbacks
    if key == "ssot.db":
...
DAEMON_VERSION = "1.0"
DAEMON_PORT = "P5"
EVENT_TYPE_GITOPS = "hfo.gen90.gitops.sync"
EVENT_TYPE_ERROR = "hfo.gen90.gitops.error"

ALLOWED_ROOT_FILES = [
    "AGENTS.md", ".env.example", ".gitignore", ".gitattributes",
    "hfo_pointers.py", "LICENSE", "README.md"
]
...
class GitOpsDaemon:
    ...
    def get_safe_files_to_commit(self) -> List[str]:
        ...
        for line in stdout.splitlines():
            ...
            is_safe = False
            if file_path.startswith("hfo_gen_90_hot_obsidian_forge/0_bronze/"):
                is_safe = True
