#!/usr/bin/env python3
"""
hfo_octree_daemon.py — HFO Gen90 Octree Daemon Swarm Engine
...
INVARIANTS_PATH = HFO_ROOT / "hfo_gen_90_hot_obsidian_forge" / "2_gold" / "2_resources" / "hfo_legendary_commanders_invariants.v1.json"
...
## Operating Context
- Gen90 SSOT: 9,860 documents, ~9M words, 9,600+ stigmergy events
...
class OctreeDaemon:
    ...
    def _p0_true_seeing_tick(self) -> None:
        ...
        try:
            import sys as _sys
            _res_path = str(
                HFO_ROOT / "hfo_gen_90_hot_obsidian_forge"
...
Keep your response under 200 words. Sign as [{self.config.port_id}:{self.config.commander}]."""
        ...
        # The router writes its own stigmergy event (hfo.gen90.llm_router.inference).
    ...
    def _pull(model_name):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _write_stigmergy("hfo.gen90.daemon.model_pull", {
...
def write_daily_search_reminder():
    ...
    _write_stigmergy(
        "hfo.gen90.daemon.model_search_reminder",
        {
...
def main():
    ...
    _write_stigmergy("hfo.gen90.daemon.swarm_start", {
        "ports": port_ids,
    ...
    try:
        ...
        _write_stigmergy("hfo.gen90.daemon.swarm_stop", {
            "ports": port_ids,
