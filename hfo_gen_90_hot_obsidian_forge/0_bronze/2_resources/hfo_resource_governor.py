"""
hfo_resource_governor.py — HFO Gen90 Resource Governor
...
class ResourceGovernor:
    ...
    def enforce(self, snapshot: Optional["ResourceSnapshot"] = None) -> None:
        ...
        3. Write hfo.gen90.governor.cycle stigmergy event
        """
        snap = snapshot or get_resource_snapshot()
        ...
        with self._lock:
            ...
            self._write_stigmergy("hfo.gen90.governor.cycle", cycle_data)

            if evicted_models:
                self._write_stigmergy("hfo.gen90.governor.evicted", {
    ...
    def reap_ghost_processes(
        ...
        for proc in excess:
            ...
            if mock_process_list is not None:
                ...
                reaped.append(pid)
                self._write_stigmergy("hfo.gen90.governor.ghost_reaped", {
