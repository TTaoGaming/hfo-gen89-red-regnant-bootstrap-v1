...
def fail_closed(reason: str, details: dict = None):
    ...
    # Write to SSOT
    _write_p5_event("hfo.gen90.p5.fail_closed", {
        "reason": reason,
        "details": details or {},
        "severity": "CRITICAL",
    })
...
def generate_eval_receipt(
    ...
    _write_p5_event("hfo.gen90.p5.eval_receipt", receipt)
...
def run_governed_eval(model: str, mode: str, limit: int, verbose: bool = True) -> list[dict]:
    ...
    _write_p5_event("hfo.gen90.p5.eval_preflight", {
    ...
    # Import eval harness
    sys.path.insert(0, str(HFO_ROOT / "hfo_gen_90_hot_obsidian_forge" / "0_bronze" / "2_resources"))
    ...
    for mdl in models:
        ...
        if mode in ("raw", "both"):
            try:
                ...
                _write_p5_event("hfo.gen90.p5.eval_error", {
    ...
    # Write audit summary
    _write_p5_event("hfo.gen90.p5.eval_audit", {
...
def phoenix_recovery(verbose: bool = True):
    ...
    _write_p5_event("hfo.gen90.p5.phoenix_recovery", {
        "actions": actions,
