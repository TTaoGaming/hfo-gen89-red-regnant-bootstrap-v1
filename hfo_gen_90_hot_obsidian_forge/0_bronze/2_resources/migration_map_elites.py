"""
MAP-Elites Behavioral Grid Migration for HFO Gen90 SSOT
========================================================
...
Schema ID: hfo.gen90.mission_state.map_elites.v1
"""
import sqlite3
import hashlib
import json
...
DB_PATH = os.path.join(
    HFO_ROOT,
    "hfo_gen_90_hot_obsidian_forge", "2_gold", "resources",
...
BEGIN
    ...
    VALUES (
        'hfo.gen90.mission.state_created',
...
BEGIN
    INSERT INTO stigmergy_events (event_type, timestamp, source, subject, data_json, content_hash)
    VALUES (
        'hfo.gen90.mission.state_changed',
        strftime('%Y-%m-%dT%H:%M:%f+00:00', 'now'),
        'mission_state_trigger',
        NEW.thread_key,
        json_object(
...
BEGIN
    INSERT INTO stigmergy_events (event_type, timestamp, source, subject, data_json, content_hash)
    VALUES (
        'hfo.gen90.mission.state_deleted',
        strftime('%Y-%m-%dT%H:%M:%f+00:00', 'now'),
        'mission_state_trigger',
        OLD.thread_key,
        json_object(
...
def run_migration(db_path: str, dry_run: bool = False):
    ...
    """, (json.dumps({
        "schema_id": "hfo.gen90.mission_state.map_elites.v1",
        "created": NOW,
        "behavioral_descriptors": ["port", "era", "thread", "meadows_level"],
        "fitness_range": "0.0-1.0",
        "mutation_confidence_range": "0-100",
        "seed_source": "braided_mission_thread_alpha_omega_hfo_gen90v8.yaml + REFERENCE_OBSIDIAN_HOURGLASS_TRELLIS",
    ...
    c.execute("SELECT COUNT(*) FROM stigmergy_events WHERE event_type LIKE 'hfo.gen90.mission.%'")
