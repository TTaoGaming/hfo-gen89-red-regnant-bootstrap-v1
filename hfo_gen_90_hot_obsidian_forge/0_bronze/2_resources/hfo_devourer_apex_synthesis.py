...
def gather_system_snapshot() -> dict:
    ...
    try:
        daemon_families = [
            "hfo.gen90.singer%", "hfo.gen90.dancer%", 
            "hfo.gen90.devourer%", "hfo.gen90.kraken%",
            "hfo.gen90.summoner%", "hfo.gen90.daemon%",
            "hfo.gen90.prey8%",
        ]
        daemon_summary = {}
...
You are reviewing the complete HFO (Hive Fleet Obsidian) Gen90 system architecture.
...
3. BUILD QUEUE: The Grimoire V7 maps ALL 64 spell slots across 8 champions with
   ...
   prioritized build queue for Gen90. Each spell slot = a TODO with a spec.
...
def build_user_prompt(snapshot: dict) -> str:
    ...
    if "grimoire_v7" in snapshot:
        ...
        sections.append("This is the literate programming build queue for Gen90.\n")
