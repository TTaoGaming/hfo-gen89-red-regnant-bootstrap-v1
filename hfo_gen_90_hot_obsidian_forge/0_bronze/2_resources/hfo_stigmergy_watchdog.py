...
=================================================================
v1.0 | Gen90 | P5 IMMUNIZE + P4 DISRUPT observer daemon
...
_FORGE = os.environ.get(
    "HFO_FORGE", "hfo_gen_90_hot_obsidian_forge"
)
...
DB_PATH = Path(
    os.environ.get(
        "HFO_SSOT_DB",
        str(_WORKSPACE_ROOT / _FORGE / "2_gold" / "2_resources" / "hfo_gen90_ssot.sqlite"),
    )
)
    ...
    try:
        ...
        row = conn.execute(
            """INSERT OR IGNORE INTO stigmergy_events
               ...
               VALUES (?, ?, ?, ?, ?, ?)""",
            (
                parsed.get("type", "hfo.gen90.watchdog.unknown"),
