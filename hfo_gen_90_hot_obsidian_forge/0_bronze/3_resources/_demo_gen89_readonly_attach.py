...
def demo_readonly_attach():
    ...
    gen89_db_path = root_dir / "hfo_gen_90_hot_obsidian_forge" / "0_bronze" / "3_archives" / "gen89_legacy" / "hfo_gen89_ssot.sqlite"
    ...
    print(f"Attaching Gen 89 SSOT as read-only: {gen89_db_path}")
    ...
    attach_query = f"ATTACH DATABASE 'file:{gen89_db_path.as_posix()}?mode=ro' AS gen89;"
    ...
    print("Successfully attached Gen 89 database as 'gen89'.")
    ...
    cursor.execute("SELECT COUNT(*) FROM gen89.documents")
    print("Gen 89 document count:", cursor.fetchone()[0])
    
    print("\n--- Attempting to write to Gen 89 (Should Fail) ---")
    try:
        cursor.execute("INSERT INTO gen89.meta (key, value) VALUES ('test_write', 'should_fail')")
