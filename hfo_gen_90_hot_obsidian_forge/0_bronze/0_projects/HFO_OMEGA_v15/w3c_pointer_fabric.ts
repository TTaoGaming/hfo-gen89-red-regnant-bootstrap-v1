...
export class W3CPointerFabric implements Plugin {
    ...
    private onPointerUpdate(data: any) {
        if (this.primaryHandId === null) {
            this.primaryHandId = data.handId;
        } else if (data.handId !== this.primaryHandId) {
            // LOCK STEALING: MediaPipe lost tracking and assigned a new ID.
            // If the primary hand isn't actively pinched, steal the lock.
            const state = this.activePointers.get(this.POINTER_ID_BASE + this.primaryHandId);
            if (!state || !state.isDown) {
                this.removeHand(this.primaryHandId);
                this.primaryHandId = data.handId;
            } else {
                return;
            }
    ...
    private firePointerEvent(
        ...
        if (this.config.dispatchToIframes && target.tagName.toLowerCase() === 'iframe') {
            ...
            if (iframe.contentWindow) {
                ...
                console.log('[W3CPointerFabric] postMessage to iframe:', type, 'iframeX:', iframeX, 'iframeY:', iframeY);
